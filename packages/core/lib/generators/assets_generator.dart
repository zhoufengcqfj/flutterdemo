  ...
  if (assetType.isSupportedImage) {
    ...
    if (integration == null) {
      return _Statement(
        ...
        '\'${posixStyle(assetType.path)}\''
            :'\'${posixStyle('packages/$packageName/${assetType.path}')}\'',
          ...
          .map(
            (e) => _createAssetTypeStatement(
              ...
              (e.isUniqueWithoutExtension
                  ...
                  .camelCase(),
              packageName: config.packageParameterLiteral
            ),
          )
...
String _flatStyleDefinition(
  ...
  final statements = _getAssetRelativePathList(config.rootPath, config.assets)
      ...
      .whereType<_Statement>()
      .toList();
  return _assetsClassDefinition(statements,packageName: config._packageName);
}
...
{String? packageName}) {
  final statementsBlock = statements
      ...
      .join('\n');
  var packageStatement = packageName == null ?
